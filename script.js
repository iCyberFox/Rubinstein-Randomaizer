/* ===== Отримання API ключа з Netlify Function ===== */
let API_KEY = '';
(async function() {
  try {
    ...
    if (response.ok) {
      ...
      console.error('Не вдалося отримати API ключ');
    }
  } catch (error) {
  ...
  reader.onload = function (ev) {
    try{
      ...
      for (let R = range.s.r; R <= range.e.r; R++) {
        ...
        if(name){
          participants.push({ number, name, handle, url, channelId, avatar:null });
        }
      }
      ...
      alert(`Знайдено ${allParticipants.length} учасників`);
    }catch(err){
      console.error(err);
...
document.getElementById('pickOne').addEventListener('click', async () => {
  ...
  const remaining = allParticipants.filter(p => !selectedWinners.includes(p));
